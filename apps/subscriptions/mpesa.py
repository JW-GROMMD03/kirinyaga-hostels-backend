...
# These are Safaricom's official callback IPs. Only these should be trusted.
SAFARICOM_IPS = [
    '196.201.214.200',
    '196.201.214.206',
    '196.201.214.207',
...
def verify_safaricom_ip(request):
    ...
    This stops random people from faking payment confirmations.
    In sandbox mode, we skip this check since callbacks come from your own server.
    """
...
def mpesa_callback(request):
    ...
    try:
        ...
        try:
            transaction_obj = PaymentTransaction.objects.select_related('subscription', 'subscription__owner', 'subscription__plan').get(
        ...
        if result_code == 0:
            ...
            with transaction.atomic():
                # Deactivate old subscriptions for this owner
