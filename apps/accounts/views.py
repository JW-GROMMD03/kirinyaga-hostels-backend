...
class StudentLoginSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        # ✅ Check if account exists and is blocked/locked FIRST
        try:
            ...
            if not otp_token:
                ...
                try:
                    ...
                    if not otp_obj.is_valid():
                        raise serializers.ValidationError({'message': 'OTP has expired. Please request a new one.'})
                    if otp_obj.otp != otp_token:
                        raise serializers.ValidationError({'message': 'Invalid OTP code.'})
                    otp_obj.used = True
                    otp_obj.save()
                except TwoFactorOTP.DoesNotExist:
                    raise serializers.ValidationError({'message': 'No valid OTP found. Please request a new one.'})
